...
public abstract class TaskContent {
    ...
    public Slide activitySlide = new Slide(706,168,706,447,2,500,1000);
    public Slide taskSlide = new Slide(1160,307,1160,450,2,500,1000);
    ...
    public TaskContent(AtFairyImpl mFairy){
        err = 0;
        fm = new HashMap<>();
        tm = new HashMap<>();
        once=new ArrayList<>();
    ...
    public void initTaskContent() throws Exception {
        ...
        while (mFairy.condit()) {
            ...
            if (taskContentNum == 1) {
                if (err > 1 || timeMap("errCount",30000,true)) {
    ...
    public int[] getTime(String string) throws Exception {
        if(string.equals("")) {
            return null;
        }
        String[] arrstr = string.split("\\|\\|");
        ...
        int hour = Integer.parseInt(new_arrstr[0]);
        int minute = Integer.parseInt(new_arrstr[1]);
        int second = Integer.parseInt(new_arrstr[2]);
    ...
    public FindResult rangeFindPic(int range,String ... str)throws Exception{
        FindResult resultStr = new FindResult();
        resultStr.sim =0;
        for (int i = 0; i < str.length; i++) {
            TemplateInfo info = ScProxy.assets().info(str[i]);
            result =  mFairy.findPic(info.x-range,info.y-range,info.width+info.x+range,info.y+info.height+range,str[i]);
            LtLog.e(mFairy.getLineInfo("fanwei:"+(info.x-range)+","+(info.y-range)+","+(info.width+info.x+range)+","+(info.y+info.height+range)));

            if(result.sim>resultStr.sim){
                resultStr=result;
            }
        }
        return resultStr;
 ...
 */
    ...
    class Slide {
        ...
        private int initSlideCount = 4;
        private int type;
        public Slide(int x, int y, int xx, int yy,int x1, int y1, int xx1, int yy1,int type,int moveTime,long endTime) {
            slideRangeIndex[0] = x;
            slideRangeIndex[1] = y;
        ...
        public Slide(int x, int y, int xx, int yy,int type,int moveTime,long endTime) {
            slideRangeIndex[0] = x;
            slideRangeIndex[1] = y;
            slideRangeIndex[2] = xx;
            slideRangeIndex[3] = yy;
        ...
        /****/
        public void slideRange(int num, int[] count,int init) throws Exception {
            if (count.length > 0) {
                if (num == count[0]) {
        ...
        public void slideRange(int num, int[] count) throws Exception {
            if (count.length > 0) {
