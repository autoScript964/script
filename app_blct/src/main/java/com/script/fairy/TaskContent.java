...
public abstract class TaskContent {
    ...
    public  FindResult widthfindPic(int width,String ...imageName)throws Exception{
        FindResult result = new FindResult();
        result.sim=0;
        for (int i = 0; i < imageName.length; i++) {
            TemplateInfo t = mFairy.getTemplateInfo(imageName[i]);
            FindResult  findResult = mFairy.findPic((t.x-width),(t.y-width),(t.x+t.width+width),(t.y+t.height+width),imageName[i]);

            /*LtLog.e(""+findResult.sim);
            LtLog.e((t.x-width)+","+(t.y-width)+","+(t.x+t.width+width)+","+(t.y+t.height+width));*/

            result = findResult.sim>result.sim?findResult:result;
        }
        return result;
    }
